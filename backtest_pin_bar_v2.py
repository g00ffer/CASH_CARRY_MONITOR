#!/usr/bin/env python3
"""
backtest_pin_bar_v2.py — Пин-бар на сильных уровнях Герчика (Н1).

Отличия от v1:
- Уровни определяются через GerchikLevelDetector (кластеризация + скоринг)
- Только уровни с высоким скорингом (>50)
- Фильтр по ложным пробоям (уровень должен иметь >=1 ложный пробой)

Стратегия:
1. Для каждой монеты: прогоняем GerchikLevelDetector на истории
2. Берём только сильные уровни (скоринг >= 50)
3. Ищем пин-бары на этих уровнях
4. Вход на закрытии пин-бара, стоп за тенью, тейк 2× стопа
...
def cluster_pivots(pivots: list[dict], atr: float,
                   eps_mult: float = CLUSTER_ATR_MULT) -> list[dict]:
    """Простая кластеризация пивотов по цене (без DBSCAN для скорости)."""
    if not pivots:
        return []
...
def detect_levels(highs: np.ndarray, lows: np.ndarray, closes: np.ndarray,
...
def simulate_symbol(symbol: str, times: np.ndarray, opens: np.ndarray,
    ...
    """Симуляция стратегии пин-бара на сильных уровнях."""
    n = len(closes)
    if n < 200:
        return []
    
    # Обнаруживаем сильные уровни на всей истории
    levels = detect_levels(highs, lows, closes)
    if not levels:
        return []
    ...
    for i in range(100, n):
        ...
        for level in levels:
            ...
            zone_width = zone_high - zone_low
            tolerance = zone_width * 0.5 + zone_low * 0.002  # допуск
...
def main():
    print("=" * 85)
    print("PIN BAR BACKTEST v2 — пин-бар на уровнях Герчика (Н1)")
    print(f"  Уровень: скоринг ≥ {SCORE_THRESHOLD}, касаний ≥ {MIN_TOUCHES}, ложных пробоев ≥ {MIN_FALSE_BREAKOUTS}")
