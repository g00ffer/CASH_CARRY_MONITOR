#!/usr/bin/env python3
"""
visualize_trades_v4.py — Визуализация нескольких сделок из бэктеста v4.

Показывает:
- 2 лучших прибыльных pin-бара
- 2 лучших прибыльных рельсы
- 2 худших убыточных pin-бара
- 2 худших убыточных рельсы

Итого 8 графиков с полной информацией:
- Свечи H1
- Зона уровня Герчика (жёлтая)
- Вход (синяя стрелка)
- Стоп (красная пунктирная)
- Тейк (зелёная пунктирная)
- Выход (зелёная/красная стрелка)
- Аннотации (триггер, скоринг, PnL)

Run:
  python visualize_trades_v4.py
"""
...
OUTPUT_DIR = Path("visualizations_v4")

# Параметры v4 baseline
LEFT_BARS = 5
RIGHT_BARS = 5
ATR_PERIOD = 14
...
def close_trade(t, i, times, closes, price, reason):
    t['exit_idx'] = i
    t['exit_time'] = times[i]
    t['exit_price'] = price
...
def plot_trade(symbol, times, opens, highs, lows, closes, trade, output_path, idx_in_set):
    ...
    title = (
        f"{symbol}  |  {trigger_label} (score={level['score']:.0f})  |  "
        f"{entry_dt} → {exit_dt}  |  "
...
def main():
    ...
    print("=" * 85)
    print("VISUALIZATION v4 — подборка сделок")
    print("=" * 85)
    ...
    # Выбор 2 лучших и 2 худших из каждого типа
    selected = []
    ...
    for i, (category, trade) in enumerate(selected, 1):
        ...
        plot_trade(sym, times, opens, highs, lows, closes, trade, output_path, i)

    print(f"\n✅ Все графики сохранены в {OUTPUT_DIR}/")
    print("\nСкачать на локалку:")
    print(f"  scp gooffer@r1237472:~/cash-carry-monitor/visualizations_v4/*.png ~/Downloads/")
