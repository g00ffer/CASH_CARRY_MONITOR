#!/usr/bin/env python3
"""
backtest_pin_bar_v7.py — Пин-бар/рельсы на уровнях Герчика + фильтр аномалий.
...
- Подход: красные свечи (2 из 3 для пина, 1 из 3 для рельс);
  история подхода для рельс считается ДО красной свечи (с бара i-2),
  снижение цены оценивается до закрытия триггера (бар i)
- Пин-бар: любой цвет, тень >= 2x тела, тело в верхней половине;
  доджи: нижняя тень >= 60% диапазона = сразу валиден
- Рельсы: красная -> зелёная, слияние свечей = пин-бар
- Тень в зоне; вход−стоп <= 2x фона (волатильность, НЕ % от цены)
...
# Макс. расстояние вход→стоп в свечах фона (волатильность, НЕ % от цены)
...
def is_pin_bar(o, h, l, c, shadow_ratio=PIN_SHADOW_RATIO):
    ...
    Доджи (body <= 0): нижняя тень >= 60% диапазона = сразу валиден."""
    if h <= l or l <= 0:
        return False
    body = abs(c - o)
    rng = h - l
    ...
    # FIX 1: доджи с нижней тенью >= 60% диапазона валиден сразу,
    # без проверки верхней тени (иначе требование неявно росло до 70%)
    if body <= 0:
...
def red_approach(opens, closes, trigger_idx, is_rails, bars, min_red):
    """
    FIX 2: красный подход с раздельными точками оценки.
    - История подхода: для рельс с бара trigger_idx-2 (до красной свечи),
      для пина с бара trigger_idx-1 (до пина)
    - Снижение цены оценивается до ЗАКРЫТИЯ триггера (closes[trigger_idx]),
      чтобы зелёная свеча рельсов не маскировала восходящий импульс
    """
    start_history_idx = trigger_idx - 2 if is_rails else trigger_idx - 1

    if start_history_idx < bars:
        return False

    red_count = sum(
        1 for k in range(bars)
        if closes[start_history_idx - k] < opens[start_history_idx - k]
    )
    if red_count < min_red:
        return False

    # Цена начала подхода выше закрытия триггера (реальное снижение к входу)
    return closes[start_history_idx - bars + 1] > closes[trigger_idx]
...
def simulate_symbol(symbol, times, opens, highs, lows, closes):
    ...
    # Фон для фильтра аномалий и капа стопа: медианный диапазон за 100 баров
    ranges = highs - lows
    ...
    for i in range(LEVEL_WINDOW_H, n):
        ...
        if is_pin_bar(opens[i], highs[i], lows[i], closes[i]) and \
           red_approach(opens, closes, i, False, APPROACH_BARS, MIN_RED_BARS_PIN):
            trigger = 'pin'
            trigger_low = lows[i]
        ...
        if trigger is None and i >= 1 and \
           ...
           red_approach(opens, closes, i, True, APPROACH_BARS, MIN_RED_BARS_RAILS):
            trigger = 'rails'
        ...
        # ── Стоп не раздут: вход не выше 2× фона над тенью ──────────
        # (волатильность, не % от цены — работает на $100 и на $0.001)
        if closes[i] - trigger_low > MAX_ENTRY_STOP_ATR * background[i]:
            continue
...
def main():
    print("=" * 85)
    print("PIN BAR BACKTEST v7 — фиксы: доджи + красный подход для рельсов")
    print(f"  Уровень: окно {LEVEL_WINDOW_H}h, score ≥ {SCORE_THRESHOLD}")
    print(f"  Вход−стоп ≤ {MAX_ENTRY_STOP_ATR}× фона (волатильность, не % цены)")
    ...
    print("\n" + "=" * 85)
    print("📊 РЕЗУЛЬТАТЫ (v7 baseline + фиксы)")
    print("=" * 85)
    ...
    print("\n" + "=" * 85)
    print("✅ Готово. Это v7 с фиксами — дальше по одному параметру за запуск")
    print("=" * 85)
